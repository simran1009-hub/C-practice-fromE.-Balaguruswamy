...
int main(){
    int mark[7], count=0;
    printf("This program counts the number of students who have obtained marks between 41 to 60\n. The marks entered should lie between 0 to 100 only\n");
    for (int i = 0; i < 7; i++)
    {
        printf("Enter mark[%d]\n",i);
        scanf("%d",&mark[i]);
       ...
       if(mark[i]>=41 && mark[i]<=60){
            count++;
        }
    }
    printf("%d number of students have scored in the range of 41 to 60\n",count);
